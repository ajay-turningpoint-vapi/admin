...
import { ReelsDelete, ReelsGet, SetReelsObj } from '../../redux/actions/Reels/reels.actions';
import { toastError, toastSuccess } from '../Utility/ToastUtils';
import { deleteMultipleReels, deleteReelsById } from '../../services/reels.service';
...
const Reels = () => {
  ...
  useEffect(() => { console.log(reelsArr, "reelsArr") }, [reelsArr])
  ...
  const contest_columns = [
    {
      name: "SL",
      cell: (row, index) => <input key={index} type="checkbox" checked={row.checked} onChange={(e) => handleCheckReels(row, index, e)} />,
      sortable: true,
      width: "2%",
    },
    // {
    ...
    },
    {
      ...
      selector: (row) => row?.displayLikeAfter ? `${row?.displayLikeAfter}s` : "N.A.",
      width: "25%",
    },
    {
      name: "Image/Video",
      ...
      cell: (row) => <div>
        {
          ...
          row.isVideo ?
            <video width="220" height="150" src={generateFilePath(row.fileUrl)} controls autoPlay={false}>
            </video>
            // <video source={generateFilePath(row.fileUrl)}></video>
            :
            <img height="84px" width="56px" alt={row.name} src={generateFilePath(row.fileUrl)} />
        }
      </div>
    },
    {
      name: "Action",
      width: "10%",
      cell: (row) => <ActionIcon isRedirected={true} onEditClick={() => handleEdit(row)} editPath={`/Reels/edit/${row._id}`} onDeleteClick={() => {
      ...
      } deletePath="/Reels/View" remove edit Uniquekey={row.id} />,
    },
  ];
        ...
        <div className="container-fluid p-0">
          <div className="row">
            <div className="col-12">
              ...
              <DashboardTable>
                <DataTable columns={contest_columns} data={reelsArr && reelsArr.length > 0 ? reelsArr : []} pagination />
              </DashboardTable>
            </div>
          </div>
        </div>
